...
extern int bionic_setjmp(void *env);
extern void bionic_longjmp(void *env, int value);
extern int bionic_sigsetjmp(void *env, int save_mask);
extern void bionic_siglongjmp(void *env, int value);
...
/* Bionic FILE objects are not layout-compatible with newlib/SceLibc FILE.
 * Soda Dungeon uses setbuf only as an optional buffering hint, so ignoring it
 * is safer than handing an Android stream object to the Vita C library. */
static void setbuf_soloader(FILE *stream, char *buffer)
{
    (void)stream;
    (void)buffer;
}

static int is_bionic_standard_stream(FILE *stream)
{
    uintptr_t p = (uintptr_t)stream;
    uintptr_t begin = (uintptr_t)&__sF_fake[0];
    uintptr_t end = begin + sizeof(__sF_fake);
    return p >= begin && p < end;
}

/* Android's __sF objects are only placeholders in this loader; their FILE
 * layout is not compatible with either Vita libc. Never pass one into an
 * implementation that dereferences private FILE callbacks. */
static int fseek_soloader(FILE *stream, long offset, int whence)
{
    if (!stream || is_bionic_standard_stream(stream))
        return -1;
#ifdef USE_SCELIBC_IO
...
static long ftell_soloader(FILE *stream)
{
    if (!stream || is_bionic_standard_stream(stream))
        return -1;
#ifdef USE_SCELIBC_IO
...
static int fseeko_soloader(FILE *stream, off_t offset, int whence)
{
    return fseek_soloader(stream, (long)offset, whence);
}

static off_t ftello_soloader(FILE *stream)
{
    return (off_t)ftell_soloader(stream);
}

static void rewind_soloader(FILE *stream)
{
    if (stream && !is_bionic_standard_stream(stream))
        (void)fseek_soloader(stream, 0, SEEK_SET);
}

extern so_module so_mod;
extern so_module so_mod_game;

/*
 * hxcpp asks Lime for this callback through dlsym during startup.  The Vita
 * loader has no moving-GC integration, but the game only needs the callback
 * to exist; its stack roots are retained by the single process loader.
 */
static void gc_set_top_of_stack(void)
{
}
...
void *dlsym_soloader(void * handle, const char * symbol) {
    ...
    if (strcmp(symbol, "gc_set_top_of_stack") == 0)
        return (void *)&gc_set_top_of_stack;

    // Lime loads CFFI entry points from libApplicationMain.so at runtime.
    // Both Android libraries are already mapped by the loader, so resolve
    // their dynamic exports instead of pretending that dlsym always fails.
    uintptr_t address = so_symbol(&so_mod_game, symbol);
    if (!address)
        address = so_symbol(&so_mod, symbol);
    if (address)
        return (void *)address;
...
so_default_dynlib default_dynlib[] = {
        ...
        { "setbuf", (uintptr_t)&setbuf_soloader },
        ...
        { "sigsetjmp", (uintptr_t)&bionic_sigsetjmp },
        { "siglongjmp", (uintptr_t)&bionic_siglongjmp },
