    ...
    for (int i = 0; i < mod->num_dynamic; i++) {
        switch (mod->dynamic[i].d_tag) {
            case DT_NEEDED:
            {
                ...
                while (curr) {
                    /*
                     * Android libraries are not required to carry DT_SONAME.
                     * Soda Dungeon's two libraries omit it, so a dependency
                     * lookup must skip those modules instead of passing NULL
                     * to strcmp before falling back to default_dynlib.
                     */
