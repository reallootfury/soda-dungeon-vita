...
/*
 * Return stable non-NULL IDs for Java methods and fields not covered by the
 * explicit per-game registry. hxcpp treats a NULL method ID as fatal; the
 * dispatchers below safely return a type-appropriate default for these sink
 * IDs while later logs show which real implementation is still needed.
 */
#define FALSOJNI_DYNAMIC_ID_BASE 0x10000

static jmethodID falsojni_generate_method_id(const char *name) {
    unsigned long hash = 5381;
    int c;
    const char *p = name;
    while ((c = *p++))
        hash = ((hash << 5) + hash) + (unsigned char)c;
    return (jmethodID)(unsigned long)(FALSOJNI_DYNAMIC_ID_BASE | (hash & 0x7FFF));
}

static jfieldID falsojni_generate_field_id(const char *name) {
    unsigned long hash = 5381;
    int c;
    const char *p = name;
    while ((c = *p++))
        hash = ((hash << 5) + hash) + (unsigned char)c;
    return (jfieldID)(unsigned long)(FALSOJNI_DYNAMIC_ID_BASE | (hash & 0x7FFF));
}
...
jfieldID getFieldIdByName(const char* name) {
    ...
    fjni_logv_warn("Unknown field name \"%s\" — using sink ID", name);
    return falsojni_generate_field_id(name);
}
...
jmethodID getMethodIdByName(const char* name) {
    ...
    fjni_logv_warn("Unknown method name \"%s\" — using sink ID", name);
    return falsojni_generate_method_id(name);
}
